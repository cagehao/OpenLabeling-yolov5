...
from yolov5.detect import detect_yolov
...
def detect_image(on):
    if on:
        dets, imgsz = detect_yolov(image_path=IMAGE_PATH_LIST[img_index],annopath = annotation_paths[1])
        print(dets)
        # for det in dets:
        #     # det [xmax,xmin,ymax,ymin,conf,cls]
        #     # y[:, 0] = (x[:, 0] + x[:, 2]) / 2  # x center
        #     # y[:, 1] = (x[:, 1] + x[:, 3]) / 2  # y center
        #     # y[:, 2] = x[:, 2] - x[:, 0]  # width
        #     # y[:, 3] = x[:, 3] - x[:, 1]  # height
        #     [ ymax, xmax, ymin, xmin, conf, class_index] = det
        #     xmax, xmin, ymax, ymin, conf, class_index = int(xmax/imgsz[0]*width), int(xmin/imgsz[0]*width), int(ymax/imgsz[1]*height), int(ymin/imgsz[1]*height), conf, int(class_index)
        #     print(xmax, xmin, ymax, ymin, conf, class_index)
        #     img_objects.append([class_index, xmin, ymin, xmax, ymax])
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar('Detect',WINDOW_NAME,0,1,detect_image)
    # selected class
